...
class ProPhotoSorter(ctk.CTk):
    ...
    def show_photo(self):
        ...
        # Load & Ambil EXIF Metadata
        self.read_metadata(file_path, filename)
        
        try:
            self.raw_image = Image.open(file_path)
            # Batasi cache awal 1600px agar render super gesit & tidak lag
            self.raw_image.thumbnail((1600, 1600), Image.Resampling.NEAREST)
            self.cached_preview = self.raw_image.copy()
            self.render_image()
        except Exception:
            self.canvas.delete("all")
    ...
    def read_metadata(self, file_path, filename):
        ...
        try:
            ...
            pass # Lewati jika file tidak memiliki skema EXIF standar (seperti hasil download internet)
    ...
    def render_image(self):
        ...
        # Pindah ke mode Lanczos super tajam saat posisi normal (Fit Screen)
    ...
    def move_photo(self, rating_key):
        ...
        # Efek Visual Flash: Beri warna menyala pada indikator angka atas saat ditekan
    ...
    def flash_indicator(self, key):
        # Mengubah warna box angka di bagian atas secara instan untuk umpan balik visual
        colors = {"1": "#2ecc71", "2": "#3498db", "3": "#9b59b6", "4": "#f1c40f", "5": "#e74c3c"}
        self.ind_buttons[key].configure(fg_color=colors[key], text_color="white")
        # Kembalikan ke warna gelap semula setelah 150 milidetik
